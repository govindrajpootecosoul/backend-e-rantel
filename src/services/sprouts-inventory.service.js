...
const getSkuUnitCosts = async () => {
  const rows = await SproutsChainStore.aggregate([
...
exports.getFilterOptions = async () => {
  const months = await SproutsInventory.distinct('reportMonth');
...
exports.getDashboard = async (reportMonth = 'All') => {
  ...
  const rows = await SproutsInventory.find(match).select('-__v').lean();
