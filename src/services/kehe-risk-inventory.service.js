...
exports.getFilterOptions = async (filters = {}) => {
  ...
  const [result] = await KeheRiskInventory.aggregate(pipeline);
...
exports.getDashboard = async (filters = {}) => {
  ...
  const riskRows = await KeheRiskInventory.find(match).select('-__v').lean();
  ...
  const inventoryRows = normalizeInventoryRows(await KeheInventory.find(invBaseQuery).lean());
